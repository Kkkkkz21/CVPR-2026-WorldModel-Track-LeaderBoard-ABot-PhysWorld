...
class BaselineWMTrainer(Trainer):
    def get_models(self, model_config):
        ...
        if model_config.get("unpretrain", False):
            ...
            # from_pretrained 可能使用 meta device 延迟加载，需要先实体化所有参数到 CPU
            # 避免后续 deepcopy 和 EMA 遇到 meta tensor 报错
            for module in transformer.modules():
                for param_name, param in list(module._parameters.items()):
                    if param is not None and param.is_meta:
                        # 用 zeros_like 而非 empty_like，避免随机内存垃圾值污染权重
                ...
                for buf_name, buf in list(module._buffers.items()):
                    if buf is not None and buf.is_meta:
                        module._buffers[buf_name] = torch.zeros_like(buf, device='cpu')
            # replay patch embedding 从 patch_embedding deepcopy 初始化
            transformer.replay_patch_embedding = copy.deepcopy(transformer.patch_embedding)
            # condition_blocks 从主干对应注入点层 deepcopy 初始化（每个注入点一个 block）
            for condition_block_idx, inject_layer_idx in enumerate(transformer.condition_inject_layer_indices):
                transformer.condition_blocks[condition_block_idx] = copy.deepcopy(
                    transformer.blocks[inject_layer_idx]
                )
            # condition_before_proj 和 condition_zero_linears 不在 checkpoint 里，
            # 被上面的 zeros_like 实体化为全零，这正是所需的零初始化，无需额外处理
        # 冻结主干参数，只训练 condition 相关模块
        for param in transformer.parameters():
            param.requires_grad = False
        # condition 相关模块全部参与训练（主干 blocks 冻结）
        trainable_modules = [
            transformer.replay_patch_embedding,
            transformer.condition_before_proj,
            transformer.condition_blocks,
            transformer.condition_zero_linears,
        ]

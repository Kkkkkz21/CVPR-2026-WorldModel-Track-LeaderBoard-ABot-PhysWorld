...
class PromptTokenizerTransform:
    ...
    def __init__(
        ...
        # Workaround for newer transformers versions that fail to load PreTrainedTokenizerFast
        # via AutoProcessor.from_pretrained when sentencepiece/tiktoken is not installed.
        # Directly load tokenizer.json and instantiate UniversalActionProcessor manually.
        import json as _json
        import importlib.util as _importlib_util
        from transformers import PreTrainedTokenizerFast as _PreTrainedTokenizerFast
        import os as _os
        _tokenizer_json_path = _os.path.join(fast_tokenizer_path, "tokenizer.json")
        _processor_config_path = _os.path.join(fast_tokenizer_path, "processor_config.json")
        _processing_module_path = _os.path.join(fast_tokenizer_path, "processing_action_tokenizer.py")
        _spec = _importlib_util.spec_from_file_location("processing_action_tokenizer", _processing_module_path)
        _processing_module = _importlib_util.module_from_spec(_spec)
        _spec.loader.exec_module(_processing_module)
        _bpe_tokenizer = _PreTrainedTokenizerFast(tokenizer_file=_tokenizer_json_path, clean_up_tokenization_spaces=False)
        with open(_processor_config_path, "r") as _f:
            _processor_config = _json.load(_f)
        self.fast_tokenizer = _processing_module.UniversalActionProcessor(
            _bpe_tokenizer,
            scale=_processor_config.get("scale", 10),
            vocab_size=_processor_config.get("vocab_size", 2048),
            min_token=_processor_config.get("min_token", 0),
        )
